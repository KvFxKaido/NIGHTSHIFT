...
test("visual scale and yaw become shared solid dimensions with the correct handedness", async () => {
  ...
  const sim = createSim("fwd", { ...createAlderWorld(), solids: resolved.blocks }, { traffic: false });
  try {
    sim.world.step();
    let hit = false;
...
test("invalid and stale imports cannot silently replace the layout", () => {
  ...
  assert.throws(() => resolveAlderLayout({ schema: 1, baseline: "old", buildings: [{ ...source, id: sourceId }] }), /district changed/);
  const upgraded = resolveAlderLayout({ schema: 1, baseline: ALDER_LAYOUT_BASELINE, buildings: [{ ...edited, id: sourceId }] });
  assert.deepEqual(upgraded.layout, layout());
  assert.throws(() => resolveAlderLayout({ ...layout(), retired: [GARAGE_PLOT_ID] }), /Garage is fixed/);
