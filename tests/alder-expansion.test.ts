...
import extension from "../assets/maps/alder/east-hills-layout.json" with { type: "json" };
import { ALDER_DATA, ALDER_TREES, ALDER_STREETS, alderHeight, createAlderWorld } from "../src/sim/alder.ts";
...
test("expanded connected streets enclose at least ten square kilometres without terrain padding", () => {
  const points=ALDER_DATA.roads.flatMap(road=>road.points);
  const area=hullArea(points)/1e6;
  assert.ok(area>=10,`street footprint only ${area} km²`);
  assert.ok(Math.abs(area-ALDER_DATA.roadHullKm2)<.0001);
  const graph=new Map<string,string[]>();
  for(const street of ALDER_STREETS) for(const [from,to] of [[street.from,street.to],[street.to,street.from]]) {
    graph.set(from!,[...graph.get(from!)??[],to!]);
  }
  const seen=new Set<string>(),todo=[ALDER_STREETS[0]!.from];
...
test("expansion preserves released southwest roads, plots and their grades", () => {
  assert.deepEqual(ALDER_DATA.roads.slice(0,released.roads.length),released.roads);
  // Released plots keep their identities and order; the only ones allowed to
  // go are those an authored alley cuts through (assets/maps/alder/alleys.json).
  const alleyCorridors=ALDER_DATA.roads.filter(r=>r.id.startsWith("sea-alley-")).map(r=>({points:r.points,reach:r.width/2+2.8}));
  ...
  for(const road of released.roads)for(const [x,z] of road.points) {
    assert.equal(alderHeight(x!,z!),2+34*smooth((x!+50)/640)*smooth((330-z!)/500));
  }
  for(const tree of ALDER_TREES)assert.ok(createAlderWorld().solids!.includes(tree));
  assert.ok(alderHeight(-730,-2420)>50,"Queen Anne needs a distinct climb");
  assert.ok(alderHeight(1150,-2240)>80,"Capitol Hill needs its high ground");
});
...
test("traffic height reaches the exact endpoint on graded lanes",()=>{
  ...
  for(const lane of network.lanes) {
    ...
    assert.ok(Math.abs(pose.y-alderHeight(pose.x,pose.z))<.00001,`lane ${lane.id} endpoint height`);
  }
});
for(const name of ["Queen Anne Climb","Ridge Scenic Way"])test(`the real rival drives ${name} through the new hills`,()=>{
  ...
  for(let i=1;i<authored.points.length;i++) {
    ...
    for(let j=i===1?0:1;j<=count;j++) {
      ...
      points.push({x,z,y:alderHeight(x,z),width:authored.width,zone:"old-quarter"});
    }
  }
  ...
  const sim=createSim("fwd",createAlderWorld(),{traffic:false,race,rival});
  try {
