...
const bytes = await readFile(new URL("../public/assets/cars/ns-kestrel-01.glb", import.meta.url));
async function car() {
  const gltf = await new GLTFLoader().parseAsync(bytes.buffer.slice(bytes.byteOffset, bytes.byteOffset + bytes.byteLength), "");
  return createBlenderCar(gltf.scene, "ns-kestrel-01", "ns-kestrel");
}

test("Kestrel ships valid compact glTF and fits the car adapter without correction", async () => {
  ...
  assert.equal(view.car.userData.model, "ns-kestrel");
...
test("Kestrel wheel centres and swept tyre surfaces clear the coachwork", async () => {
  const view = await car();
