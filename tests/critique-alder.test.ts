...
test("the Port Alder critique runs and reports legs, streets and gate candidates", () => {
  const out = execFileSync(process.execPath,
    ["--experimental-strip-types", "--no-warnings", "scripts/critique-alder.ts", "--json"],
  ...
  assert.deepEqual(report.streets.map(s=>s.id).sort(),ALDER_STREETS.map(s=>s.id).sort());
