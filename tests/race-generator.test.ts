...
// were computed on the code before and after and did not move. A map rename does
// not have to move a draw: the yard's east gate took drift-yard to v2 on
...
const WORLD = "alder-slice-v8-evergreens-v1-broadcast-v2-drift-yard-v3-arena-v1-corners-v1-market-v1-scale-v1-fronts-v2-b7922ff4-parking-v1-1e83ba00-grounds-v1-ce441b97-garage-v2-clearance-6b7ea50b";
const FINGERPRINTED: Record<"plain" | "turf", Record<GeneratedKind, { revision: string; world: string; fingerprint: string }>> = {
  plain: {
    ...
    circuit: { revision: "generator-v2", world: WORLD, fingerprint: "40c6bb23" },
    unordered: { revision: "generator-v1", world: WORLD, fingerprint: "154c50e7" },
  },
  turf: {
    sprint: { revision: "generator-v1", world: WORLD, fingerprint: "1c1c87a4" },
    circuit: { revision: "generator-v2", world: WORLD, fingerprint: "43756cf2" },
    unordered: { revision: "generator-v1", world: WORLD, fingerprint: "2ecc657c" },
  },
};
function drawFingerprint(set: "plain" | "turf", kind: GeneratedKind): string {
  ...
  if (set === "plain") {
    ...
    // drew the same 8 Moth races. So the turfs, the pull and every sampled leg's
    // share are pinned by value as well as the races. A sample is still a sample:
    // measuring the share at segment starts rather than midpoints left every
    // sampled share equal to 4 places and passed.
    parts.push(`pull ${GENERATOR.turf.pull}`, ...ALDER_TURFS.map(t => `${t.id}@${t.centre.x},${t.centre.z}r${t.radius}`));
    const addTurf = (label: string, id: string, event: ReturnType<typeof alderGeneratedRace>) => {
      add(label, event);
      parts.push(...event.generated.legs.map(leg => turfShare(graph, leg, turfFor(id)!).toFixed(4)));
    };
