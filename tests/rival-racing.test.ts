...
test("a stopped car beside its line does not draw it across, and one in its way is still dodged", () => {
  ...
  // Resting on its own side at 45 m/s, an SUV standing `beside` metres to its right 60 m on; its aim after a second.
  const aim = (beside: number) => {
    ...
    const suv = { id: 142, kind: "suv" as const, x: line + beside, y: 0, z: -160, heading: 0, speed: 0, length: 4.7 };
