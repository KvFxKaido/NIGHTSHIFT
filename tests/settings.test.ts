...
test("drivetrain and each garage category round-trip through a new store", () => {
  const disk = storage();
  const store = createSettingsStore(() => disk);
  store.update({ drivetrain: example.drivetrain });
...
test("blocked storage does not crash boot or discard choices during the session", () => {
  ...
  store.update({ drivetrain: "rwd" });
  store.update({ customization: { paint: "ice" } });
  assert.equal(store.get().drivetrain, "rwd");
...
test("a quota failure reports unsaved choices and can recover on a later change", () => {
  ...
  fail = false;
  store.update({ drivetrain: "fwd" });
  assert.equal(store.status(), "saved");
  assert.deepEqual(createSettingsStore(() => disk).get(), { ...example, drivetrain: "fwd" });
});
...
test("preview callbacks cannot write preferences or pollute the next menu edit", () => {
  ...
  store.preview(() => {
    assert.equal(store.update({ drivetrain: "awd", customization: { paint: "signal" } }), false);
  ...
  assert.equal(store.update({ drivetrain: "fwd" }), true, "preview suppression ends even when a link throws");
});
...
test("invalid user changes cannot get stored", () => {
  const disk = storage();
  const store = createSettingsStore(() => disk);
  assert.throws(() => store.update({ drivetrain: "4wd" as Drivetrain }), RangeError);
...
test("saving a choice removes only its URL override so refresh honors the saved value", () => {
  ...
  const next = new URL(withoutSettingsOverrides(href, ["paint", "drivetrain"]));
  assert.equal(next.searchParams.get("scene"), "garage");
  assert.equal(next.searchParams.get("paint"), null);
  assert.equal(next.searchParams.get("drivetrain"), null);
