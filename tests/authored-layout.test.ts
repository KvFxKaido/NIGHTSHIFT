...
import { GENERATED_SEATTLE_BLOCKS, GARAGE_PLOT_ID, SEATTLE_VERSION, resolveSeattleLayout } from "../src/sim/seattle.ts";
...
const generated = GENERATED_SEATTLE_BLOCKS;
...
test("an authored plot stands after the map is regenerated under it", () => {
  const before = resolveSeattleLayout(layout);
  ...
  const after = resolveSeattleLayout(layout, regenerated);
...
test("authored wins only over generated plots: the garage and other authored plots are issues, not displaced", () => {
  ...
  const onGarage = resolveSeattleLayout({ schema: 2, authored: [{ ...authored, x: garage.x, z: garage.z }], retired: [] });
  ...
  const pair = resolveSeattleLayout({ schema: 2, authored: [authored, { ...authored, id: "authored-2", z: authored.z + 2 }], retired: [plotId] });
...
test("the layout's identity follows its content, and an empty one leaves the world version alone", () => {
  ...
  assert.ok(!SEATTLE_VERSION.includes("-layout-"), `the checked-in layout is empty, but the world is ${SEATTLE_VERSION}`);
});
