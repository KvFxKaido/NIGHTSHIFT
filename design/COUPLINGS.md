...
As of 2026-09-24: `four-wheel-v6`, `traffic-v12` with seeds (stop and dwell at 144 dressed junctions, chains judged movement by movement, `design/INTERSECTIONS.md`), `driver-v6`,
...
| Slip term 1; traffic frame; `PASS_ASTRAY`; the will-be check | `RIVAL_STEERING`, `RIVAL_TRAFFIC_FRAME`, `RIVAL_RACING` | 09-21 and 09-22 | the batch at traffic seed 0, and Shawn's recorded races | traffic seeds; `traffic-v8`; `driver-v6` (09-25): the will-be check reads `meant` as the dodge only when one is taken | **suspect on seeds**: gated at seed 0 only. The six-seed gate (`pnpm rival:gate`) is what re-sweeps them |
...
| The ladder: street pace climbs the list | `CAR_TUNES` (`HANDLING.md`, "The ladder pass") | 09-19 | `pnpm cars --streets` at `driver-v2`, 09-22 (`HANDLING.md`, "Re-measured"): the same six sprints, clear | a generator change redraws the six sprints; `driver-v6` (the traffic column only) | **current**: the order holds, every car within 0.31 points of 09-19. The traffic column is `traffic-v7` at seed 0 |
