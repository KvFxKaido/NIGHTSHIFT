...
As of 2026-09-24: `four-wheel-v6`, `traffic-v12` with seeds (stop and dwell at 144 dressed junctions, chains judged movement by movement, `design/INTERSECTIONS.md`), `driver-v10` (the pass on a racing player, which nothing measured with the player parked sees),
`street-line-v3`, `pass-v5`, on `alder-slice-v8` (the shoulders, `design/ROAD_EDGES.md`, and Spruce Cut).

| Number | Where | Set | Measured on | Changed under it since | Status |
|---|---|---|---|---|---|
| Lane corner share 0.80 | `RIVAL_CORNERING` | 09-19 (`6f6ccc9`) | eight races clear and in `traffic-v5`, which could move a turning car 15 m in a tick; a bend-width test with steering that asked only for a turn's geometry | curved traffic corners (`traffic-v6`, 09-20); the slip feedforward (09-21, 8 m wide to 1.7 m on a fast bend); traffic seeds; claims reckoned as driven (`traffic-v8`); the dodge only for a car in the way, read where it will be (`driver-v6`, `-v7`, 09-25), which moves its in-traffic races, not the bend-width tests | **current** (re-measured 09-23, `RIVAL_CORNERING`'s note): with the slip term the 35 degree test bend is 0.96 m wide at 0.80 and 1.26 at 0.92 in the shared FWD fixture, but in the names' own cars a fast gentle bend at top speed is the limit, at full lock: the Reign 1.45 m at 0.80, 3.25 at 0.84, 6.10 at 0.86; the Vesper 2.33 at 0.80. It is the lock at speed as much as nerve. Junction arcs not re-swept |
...
| Feedforward 0.8, lead 0.3 | `RIVAL_STEERING` | 09-13 | swept 0.6 to 0.9 on Ridge's full line, with the geometry term alone | the slip term (09-21); on streets, in a lane round a bend towards the oncoming side, steering against what a car on its path sees (`chord`, `driver-v9`, 09-26), which leaves Ridge alone | **suspect**: the sweep predates the slip term |
| Slip term 1; traffic frame; `PASS_ASTRAY`; the will-be check | `RIVAL_STEERING`, `RIVAL_TRAFFIC_FRAME`, `RIVAL_RACING` | 09-21 and 09-22 | the batch at traffic seed 0, and Shawn's recorded races | traffic seeds; `traffic-v8`; `driver-v6` and `-v7` (09-25): the will-be check reads `meant` as the dodge only when one is taken, and that dodge reads the car at arrival | **suspect on seeds**: gated at seed 0 only. The six-seed gate (`pnpm rival:gate`) is what re-sweeps them |
...
| The ladder: street pace climbs the list | `CAR_TUNES` (`HANDLING.md`, "The ladder pass") | 09-19 | `pnpm cars --streets` at `driver-v2`, 09-22 (`HANDLING.md`, "Re-measured"): the same six sprints, clear | a generator change redraws the six sprints; `driver-v6`, `-v7` (the traffic column only) | **current**: the order holds, every car within 0.31 points of 09-19. The traffic column is `traffic-v7` at seed 0 |
...
| The rival gate's baseline | `measurements/rival-gate.json` | 09-24 | the 83-race batch at six traffic seeds (0, 1000, 271828, 1, 42, 314159), `traffic-v12`, `driver-v9`, `street-line-v3`, `pass-v5`, on `alder-slice-v8` (Spruce Cut), whose seeds draw other courses than v7's | nothing | **current**; re-saved by the change that moves it (`pnpm rival:gate --save`). Against v9 (v10): contact 4,869 to 1,515 ticks, resets 35 to 22, distinct incidents 50 to 52; v10 to v11: claimed crossings 8 to 3, distinct incidents 52 to 55, rear-ends 12 to 22; v11 to v12: 460 of 498 identical, claimed crossings 3 to 2; pass-v4: 449 identical, level; pass-v5: 455 identical, 17 s quicker, incidents 55 to 56 (`PORT_ALDER.md`, "What a committed pass costs"); driver-v6: 200 identical, distinct incidents 56 to 41, 123 s quicker, off the pavement 153 to 328 in four swerves round box trucks; driver-v7: 334 identical, distinct incidents 41 to 40, 42 s quicker ("A car beside its line"); driver-v9: 76 identical, distinct incidents 40 to 49, resets 26 to 15, shipped on the turn-by-turn measure ("Where the rival slows"), so these totals are the reshuffle's; driver-v10 changes only the rival racing a moving player, which the gate's parked player never is ("Passing the player in traffic"). Incidents by kind are in `measurements/stop-and-dwell.json` and `junction-dressing.json`. Its contact counts were built for traffic that was a wall; since v9 contact continues, so read its outcomes (time, resets, finishing, off the pavement). `driver-v2.json` is the seed 0 batch at `traffic-v7`, kept |
