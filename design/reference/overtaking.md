...
- **Planned slower than every re-read** (gen-81: 35 mph at the commit, 86+ from 0.2 s later). Where commit
  ...
  treats exactly that join point specially. Measured the same day: it was that, fixed in `pass-v5`. The rest of
  gen-81's loss is the in-pass emergency check holding it at the van's speed, which two changes of frame made worse,
  not better (`PORT_ALDER.md`).
