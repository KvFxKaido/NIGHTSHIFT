...
document.getElementById("map-stats")!.textContent = `${(SEATTLE_STREETS.reduce((n,s)=>n+pathLength(s.points),0)/1000).toFixed(1)} km of streets · ${SEATTLE_DATA.roadHullKm2.toFixed(1)} km² street footprint · Four open checkpoints`;
...
for (const area of SEATTLE_DATA.neighborhoods) shape("text",{x:area.x,y:area.z,"font-size":28,fill:"#c5b898","text-anchor":"middle"},area.name);
