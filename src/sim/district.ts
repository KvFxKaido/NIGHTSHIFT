import { blockCorners, blockPenetration, type BuildingBlock as DistrictBlock } from "./building-footprint.ts";
export { blockCorners, blockPenetration, type BuildingBlock as DistrictBlock } from "./building-footprint.ts";
import { projectOntoPath, type Street } from "./street-path.ts";
export { projectOntoPath, projectOntoPathUnindexed, type Street } from "./street-path.ts";
import { buildStreetTrafficNetwork } from "./street-traffic.ts";
export { buildStreetTrafficNetwork, JUNCTION_SEARCH, TRAFFIC_HEIGHT_STEP } from "./street-traffic.ts";
...
export const DISTRICT_VERSION = "blackglass-district-blockout-v4" +
  (authoredLayout.buildings.length ? `-layout-${layoutFingerprint(parseBuildingLayout(authoredLayout))}` : "");
 ...
 */
export const GENERATED_DISTRICT_BLOCKS: readonly DistrictBlock[] = DISTRICT_FACES.flatMap((face, faceIndex) => {
...
// Trigonometry may differ in its final bits between Node and the browser.
// Millimetres identify the authored geometry without rejecting the same map.
export const DISTRICT_LAYOUT_BASELINE = layoutFingerprint(GENERATED_DISTRICT_BLOCKS.map(block =>
  Object.fromEntries(Object.entries(block).map(([key, value]) => [key, Math.round(value * 1000) / 1000]))));
export const GARAGE_PLOT_ID = buildingId({ x: 269.3221008053601, z: -339.25142242330605 });

/** Re-seat a resized or moved footprint on the same ground used by the sim. */
export function groundBuilding(placement: Omit<BuildingPlacement, "id">): DistrictBlock {
  const shape = { ...placement, base: 0 };
  const grounds = [groundHeight(shape.x, shape.z), ...blockCorners(shape).map(p => groundHeight(p.x, p.z))];
  return { ...shape, base: Math.round(Math.min(...grounds) * 10) / 10 };
}

/** Resolve authored placements without mutating their generated source. */
export function resolveDistrictLayout(value: unknown): { blocks: readonly DistrictBlock[]; issues: string[] } {
  const layout = parseBuildingLayout(value);
  if (layout.buildings.length && layout.baseline !== DISTRICT_LAYOUT_BASELINE) throw new Error("The district changed. Re-export its layout before importing these edits.");
  const edits = new Map(layout.buildings.map(item => [item.id, item]));
  const known = new Set(GENERATED_DISTRICT_BLOCKS.map(buildingId));
  for (const id of edits.keys()) {
    if (!known.has(id)) throw new Error(`Unknown building: ${id}`);
    if (id === GARAGE_PLOT_ID) throw new Error("Wharf Garage is fixed in this editor version");
  }
  const blocks = GENERATED_DISTRICT_BLOCKS.map(block => {
  ...
  blocks.forEach((block, index) => {
    const id = buildingId(GENERATED_DISTRICT_BLOCKS[index]!);
    if (!edits.has(id)) return;
    if (!blockClearsStreets(block, 2.6)) issues.push(`${id}: overlaps a road or its pavement`);
    // Edge tests alone miss a small street entirely enclosed by a large footprint.
    if (DISTRICT_STREETS.some(street => street.points.some(point => footprintContains(block, point.x, point.z)))) {
      issues.push(`${id}: contains a street`);
    }
    if (!blockClearsCorridors(block)) issues.push(`${id}: overlaps the river or railway corridor`);
    if (!blockClearsWalls(block) || !blockClearsStructures(block)) issues.push(`${id}: overlaps a barrier, bridge or tunnel`);
    if (blocks.some((other, otherIndex) => otherIndex !== index &&
      Math.hypot(other.x - block.x, other.z - block.z) < spanOf(other) + spanOf(block) && blockPenetration(block, other) > 0.01)) {
      issues.push(`${id}: overlaps another building`);
    }
    const highest = Math.max(groundHeight(block.x, block.z), ...blockCorners(block).map(p => groundHeight(p.x, p.z)));
    if (highest - block.base > MAX_PLOT_DROP + 0.1) issues.push(`${id}: ground changes by more than four metres across the footprint`);
  });
  return { blocks, issues };
}

const resolvedLayout = resolveDistrictLayout(authoredLayout);
if (resolvedLayout.issues.length) throw new Error(`Invalid district layout:\n${resolvedLayout.issues.join("\n")}`);
export const DISTRICT_BLOCKS = resolvedLayout.blocks;
...
export function districtTraffic(): TrafficNetwork {
  trafficNetwork ??= buildStreetTrafficNetwork(DISTRICT_STREETS, (x, z) => projectOntoDistrict(x, z).height);
