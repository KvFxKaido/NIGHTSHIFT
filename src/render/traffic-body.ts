...
export function trafficBodyGeometry(kind: TrafficKind): { paint: THREE.BufferGeometry; detail: THREE.BufferGeometry } {
  const { length: l, width: w, height: h } = TRAFFIC_KINDS[kind];
  const painted: THREE.BufferGeometry[] = [], details: THREE.BufferGeometry[] = [];
  ...
  const box = (sx: number, sy: number, sz: number, x: number, y: number, z: number, color?: number) => {
    const g = new THREE.BoxGeometry(sx, sy, sz); g.translate(x, y, z); add(g, color);
  };
  ...
  box(w * .95, belt - sill, l, 0, (belt + sill) / 2, 0);
  ...
  return { paint: merge(painted), detail: merge(details) };
}
