...
if (!files.length) {
  console.log("No lap recordings yet. Drive a Ridge Circuit race under pnpm dev (?race=arena-full-solo) and each lap saves to recordings/laps.");
  process.exit(0);
}
...
for (const name of files) {
  ...
  report.push({ file: name, race: session.race, car: session.car, drivetrain: session.drivetrain, physics: session.physics, verified,
...
if (args.has("--json")) {
  ...
  for (const session of report) {
    ...
    console.log(`${session.file}  ${session.race} · ${session.car} ${session.drivetrain.toUpperCase()} · ${session.physics}${check}`);
