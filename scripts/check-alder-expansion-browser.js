...
async page => {
  ...
  await page.goto(base+'alder.html');
  ...
  await page.waitForFunction(()=>window.__ns?.sim.roadWorld.id==='alder-slice-v4'&&document.body.dataset.assetState==='ready',null,{timeout:60000});
  const views=[];
  for(const [name,x,z,heading] of [['queen-anne',-1040,-1665,.15],['capitol-hill',940,-2210,0],['madrona',2000,-1320,0]]) {
    views.push(await page.evaluate(async({name,x,z,heading})=>{
      const ns=__ns,{alderHeight}=await import('/src/sim/alder.ts');
      ns.sim.body.setTranslation({x,y:alderHeight(x,z)+.5,z},true);
      ...
      if(!ns.view.scene.getObjectByName('alder-park-canopies'))throw Error('Park trees missing');
      return {name,height:alderHeight(x,z),calls:ns.view.renderer.info.render.calls,triangles:ns.view.renderer.info.render.triangles};
    },{name,x,z,heading}));
