async page => {
  ...
  await page.waitForFunction(()=>window.__ns?.sim.roadWorld.id==='seattle-slice-v2');
  ...
  await page.screenshot({path:'artifacts/seattle-garage-interior.png'});
  ...
  await page.screenshot({path:'artifacts/seattle-garage-exterior.png'});
  const driving=await page.evaluate(()=>{
    ...
    if(car.z>=initial.z-40||car.speed<15)throw Error('Seattle spawn is blocked');
    if(!__ns.link().includes('world=seattle'))throw Error('Share link lost Seattle');
    if(!__ns.view.scene.getObjectByName('seattle-asphalt'))throw Error('Wrong world renderer');
  ...
  await page.screenshot({path:'artifacts/seattle-driving.png'});
  await page.evaluate(()=>__ns.go('pause'));
  await page.locator('[data-menu-screen="pause"] [data-district-map]').click();
  await page.waitForURL('**/seattle.html');
  await page.waitForFunction(()=>document.querySelectorAll('#district-map polyline').length>50);
  await page.screenshot({path:'artifacts/seattle-map.png'});
  await page.getByRole('link',{name:'Race: Sound to Sky',exact:true}).click();
  await page.waitForFunction(()=>window.__ns?.sim.state.race!==null&&window.__ns?.sim.roadWorld.id==='seattle-slice-v2');
  ...
  const terrain=await page.evaluate(async()=>{
    ...
    const {SEATTLE_STREETS,projectOntoSeattle}=await import('/src/sim/seattle.ts');
    const {resetViewCamera}=await import('/src/render/scene.ts');
    const points=SEATTLE_STREETS.flatMap(s=>s.points);
    const p=points.reduce((best,p)=>p.y>best.y?p:best,points[0]);
    const surface=projectOntoSeattle(p.x,p.z);
  ...
  await page.screenshot({path:'artifacts/seattle-hill-blockout.png'});
