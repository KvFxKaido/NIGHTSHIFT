...
const passing = process.argv.includes('--passes');
...
try {
  ...
  const report = [];
  for (const id of passing ? ['gen-7', 'gen-20', 'street-uptown-clear'] : ['gen-7', 'street-uptown', 'street-uptown-clear']) {
    ...
    const result = await page.evaluate(({ id, passing }) => {
      __ns.freeze(true);
      ...
      if (!!route.trafficPassing !== conditional) throw Error(`Wrong passing mode for ${id}`);
      if (conditional) {
        if (passing) {
          for (let i = 0; i < 720 && !__ns.sim.state.rival.driver.trafficPass; i++) __ns.drive('B10');
          if (!__ns.sim.state.rival.driver.trafficPass) throw Error('Rival never planned a pass');
          __ns.drive('B30');
        } else {
      ...
      return { id, conditional, corners: route.line?.corners.length ?? 0, blend: rival.driver.lineBlend ?? 0,
        passing: route.trafficPassing ?? false, pass: rival.driver.trafficPass ?? null,
        frame: passing ? view.renderer.domElement.toDataURL('image/png') : null,
        seconds: rival.race.ticks / 60, groundContact: rival.vehicle.groundContact };
    }, { id, passing });
    const { frame, ...details } = result;
    if (frame) await writeFile(`artifacts/traffic-pass-${id}.png`, Buffer.from(frame.split(',')[1], 'base64'));
    else await page.screenshot({ path: `artifacts/street-line-${id}.png`, timeout: 60000 });
    report.push(details);
  }
