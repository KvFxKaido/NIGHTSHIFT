...
async page => {
  ...
  if(!await page.locator('#district-map').textContent().then(text=>text.includes('Needle')))throw Error('Landmark missing from map');
  ...
  const north=await page.evaluate(()=>{
    const ns=__ns;
    if(!ns.view.scene.getObjectByName('needle'))throw Error('Missing landmark mesh');
    ...
    ns.tick(1);ns.shot();
    return {world:ns.sim.roadWorld.id,landmark:ns.view.scene.getObjectByName('needle').position.toArray()};
  });
