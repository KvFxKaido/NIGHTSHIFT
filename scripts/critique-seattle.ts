...
import { buildRoutingGraph, legTable, routeLength, PACE, RISK_WEIGHTS, type Leg, type LegClass,
  type RoutingGraph } from "../src/sim/route-choice.ts";
...
if (asJson) {
  ...
  console.log(JSON.stringify({ summary, pace: { top: PACE.top }, weights: RISK_WEIGHTS,
    streets: [...graph.measures.values()], legs: plain,
  ...
  console.log(`\nWeights: narrow ${RISK_WEIGHTS.narrow}, bends ${RISK_WEIGHTS.bends}, grade ${RISK_WEIGHTS.grade}, blind ${RISK_WEIGHTS.blind}. Pace: ${PACE.top} m/s top, 16 m streets at 94% of it, a 90 degree turn 2.5 s, a climb 1 + 1.5 x grade. Proposals. Traffic is not scored: it is seeded uniformly per lane length.`);
}
