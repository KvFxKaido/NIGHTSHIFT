/**
 * A measuring stick for Port Alder's route choice, and the material a race
 ...
 *
 *   pnpm alder:critique
 *   pnpm alder:critique --json
 */
import { ALDER_STREETS, ALDER_BLOCKS, alderHeight, createAlderWorld } from "../src/sim/alder.ts";
...
const graph: RoutingGraph = buildRoutingGraph(ALDER_STREETS, alderHeight, ALDER_BLOCKS);
...
if (tryArg) {
  ...
  const crossed = ALDER_STREETS.filter(s => s.points.slice(1).some((q, i) => crossesMidBlock(a, b, s.points[i]!, q))).map(s => s.name);
  const alley: Street = { id: "alley-try", name: "Trial Alley", from: nodeA, to: nodeB, added: true, kind: "alley",
    points: [a, b].map(p => ({ x: p.x, z: p.z, y: alderHeight(p.x, p.z), width, zone: "old-quarter" })) };
  const displaced = ALDER_BLOCKS.filter(block => segmentFootprintDistance(block, alley.points[0]!, alley.points[1]!) < width / 2 + 2.8);
  const trial = buildRoutingGraph([...ALDER_STREETS, alley], alderHeight, ALDER_BLOCKS.filter(block => !displaced.includes(block)));
  ...
  const approach = startApproach(ALDER_STREETS, createAlderWorld(true).start);
...
const summary = {
  streets: ALDER_STREETS.length, nodes: graph.nodes.length, choicePoints: graph.choicePoints.length,
...
if (asJson) {
  ...
  console.log(`Port Alder: ${summary.streets} streets, ${summary.nodes} nodes, ${summary.choicePoints} choice points, ${summary.deadEnds} dead ends`);
