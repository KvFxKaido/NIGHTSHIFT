...
async page => {
  ...
  const initial = await page.evaluate(() => {
    if (__ns.state().carModel !== 'ns-01') throw Error('Default coupe missing');
  ...
  if (await page.evaluate(() => __ns.state().carModel !== 'ns-01')) throw Error('Failure removed current car');
  ...
  const bulwark = await page.evaluate(initial => {
    ...
    if (JSON.stringify(__ns.sim.state) !== initial.vehicle) throw Error('Car selection changed simulation');
  ...
  await page.goto(base + '?scene=garage&car=blender&freeze=1'); await ready();
  if (await page.evaluate(() => __ns.state().carModel !== 'ns-01' || JSON.parse(localStorage.getItem('nightshift.settings')).car !== 'bulwark')) throw Error('Preview corrupted saved car');
  await choose('bulwark');
