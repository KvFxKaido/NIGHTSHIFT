...
- **Free roam** starts at Wharf Garage in SoDo. Stop at the shutter to
  ...
  not part of save slots). Since 2026-09-24 the rest of the setup is each car's own
  too (settings schema 4, `cars` in `settings.ts`; `lookFor` in `main.ts`): a browsed
  car is drawn and its rows read in its own look, and a save slot still keeps one
  car and that car's look, as the same JSON as before.
...
- **Venues** (2026-09-25, `design/VENUES.md`): enclosed places to race that are
  ...
  millimetre to the city's arena. Built so far: stage A. The city's arena is still
  open and Sable still parked in it; closing it is one Port Alder identity change,
  shared with Ridge Circuit becoming the track venue, Rivet's dragway beside it.
...
| Venues: the stadium world, its shell, floor and gates | `stadium.ts`, `render/stadium.ts` | `design/VENUES.md` |
...
- Customization is each car's own (settings schema 4). Whatever paints the garage platform paints `platformLook()`,
...
- The test suite takes about 5 minutes. Since 2026-09-19 CI runs it too, beside
  the build and the facade browser check, on pushes to main (master until 2026-09-25) and pull requests; it
  ran `pnpm build` alone before, so anything pushed before then was gated only by
  whoever remembered. Still run `pnpm test` yourself: CI on main tells you after
